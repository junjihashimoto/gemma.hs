...
module Gemma.Regression.FP32Spec (spec) where
...
spec :: Spec
spec = describe "FP32 PyTorch Comparison" $ do

  it "loads PyTorch reference and SafeTensors model" $ do
    -- Load PyTorch reference
    referenceJson <- BL.readFile "scripts/pytorch_fp32_reference.json"
    case JSON.eitherDecode referenceJson of
      ...
      Right (ref :: PyTorchReference) -> do
        ...
        -- Verify it's FP32
        prDType ref `shouldBe` "FP32"

        -- Define tolerance for FP32 (should be exact or near-exact)
        let tolerance = 1e-7 :: Float
